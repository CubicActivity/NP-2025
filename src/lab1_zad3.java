...
public class lab1_zad3 {
    public static void main(String[] args) {
        ...
        Supplier<Doctor> docSuplier = () -> {
            int number = sc.nextInt();
            String name = sc.next();
            int lvl = sc.nextInt();
            int numOfpatients = sc.nextInt();

            return new Doctor(number, name, lvl, numOfpatients);
        };
        for(int i=0; i<n; i++) {
            er.treat(docSuplier);
        }
        ...
        DoctorEvaluator combinedEvaluator = doctor -> doctor.getPatients() > 20 && doctor.getLevel() >= 7;
        Doctor[] qualifiedDoctors = er.evaluate(combinedEvaluator);
        for(Doctor d : qualifiedDoctors) {
        ...
        Predicate<Doctor> isChief = doctor -> doctor.getLevel() == 10;
        ...
        //TODO: Conditional mutation: increase the level of expertise of every doctor that has 30 or more patients
        System.out.println("\n=== Increase the level of expertise of every doctor by 1 ===");
        Predicate<Doctor> moreThan30Patients = doctor -> doctor.getPatients() >= 30;
        er.conditionalMutate(moreThan30Patients, doctor -> doctor.setLevel(doctor.getLevel()+1));



        //TODO: Map doctors to labels in the format: Name: name, Level: level
        System.out.println("\n=== Map doctors to labels ===");
        Function<Doctor, String> toLabel = doctor -> "Name: " + doctor.getName() + ", Level: "+doctor.getLevel();
